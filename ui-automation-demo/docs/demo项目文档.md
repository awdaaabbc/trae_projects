...
### 2.6 停止执行测试
- **接口路径**: `/api/stop-execution/:id`
- **请求方法**: `POST`
- **路径参数**: `id` (执行记录 ID, 注意不是测试用例 ID)
- **响应结构**:
  ```json
  { "ok": true }
  ```
...
- **路径参数**: `id` (测试用例 ID)
- **功能说明**:
  - 从数据库/文件系统中彻底删除指定 ID 的测试用例。
  - 同时删除该测试用例关联的所有执行记录（Executions）。
  - 此操作不可恢复（硬删除）。
- **响应结构**:
  - **成功**: HTTP 204 No Content
  - **失败 (ID不存在)**: HTTP 404 Not Found
    ```json
    { "error": "Test case not found" }
    ```
  - **失败 (参数缺失)**: HTTP 400 Bad Request
